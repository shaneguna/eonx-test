...
class ListsController extends Controller
{
    ...
    public function create(Request $request): JsonResponse
    {
        ...
        try {
            // Save list into MailChimp first to ensure that mailchimp record gets created before saving as local entity
    ...
    {
        $campaigns = $this->getLists();

        $list = array_map(function ($campaign) {
            $data[$campaign->getId()] = $campaign->toArray();
            return $data;
        }, $campaigns);

        return $campaigns === null || empty($campaigns) ? $this->errorLists() : $this->successfulResponse($list);
    }
